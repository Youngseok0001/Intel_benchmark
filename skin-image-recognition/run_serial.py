...
def create_serial_optimization(model_fn, input_fn, optimizer):
    ...
    acc = tf.reduce_mean(tf.cast(equality_train, tf.float32))
    ...
    return apply_gradient_op, loss, acc
...
def parallel_training(model_train_fn, model_test_fn, input_train_fn, input_test_fn):
    ...
    update_op, loss, acc_train = create_serial_optimization(model_train_fn, input_train_fn, optimizer)
    ...
    is_in_exclusion = lambda exclusions, x : [x 
                                              for exclusion 
                                              in exclusions 
                                              if x.op.name.startswith(exclusion) ]
